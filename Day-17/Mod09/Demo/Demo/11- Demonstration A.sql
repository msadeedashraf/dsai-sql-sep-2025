...
SELECT UnitPrice
		--, MIN(OrderQty), MAX(UnitPriceDiscount)
...
SELECT top 1 UnitPrice
		--, MIN(OrderQty), MAX(UnitPriceDiscount)
FROM Sales.SalesOrderDetail
order by UnitPrice desc --3578.27
...
select  distinct e.city from tsql.hr.Employees e

select  e.city , Count(*) from tsql.hr.Employees e
group by e.city 
...
select 
	o.custid, o.shipcity, year(o.orderdate) , count(*) 
from [TSQL].[Sales].[Orders] o 
group by o.custid, o.shipcity, year(o.orderdate)
order by 1 desc 
