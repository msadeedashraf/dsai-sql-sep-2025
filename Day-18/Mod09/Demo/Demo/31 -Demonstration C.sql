...
select p.ProductID,  p.Name, p.ProductLine FROM Production.Product AS p
...
SELECT 
p.ProductLine,COUNT(*) AS cnt 
FROM Production.Product AS p
group by p.ProductLine

select  od.OrderQty from Sales.SalesOrderDetail AS od
WHERE od.OrderQty > 20
...
-- 9 rows returned
SELECT p.ProductID,P.name,	 COUNT(*) AS cnt
FROM Production.Product AS p
JOIN Sales.SalesOrderDetail AS od
ON p.ProductID = od.Productid
